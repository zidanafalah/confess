...
function init() {
  ...
  // Teks love
function drawText() {
  // Font size responsive, maksimal 60px, minimal 20px
  const fontSize = Math.min(Math.max(width * 0.05, 20), 60);
  ctx.font = fontSize + "px Arial";
  ctx.fillStyle = "lightblue";
  ctx.textAlign = "center";

  // Posisi vertikal 80% tinggi canvas
  const yPos = mobile ? height * 0.8 : height * 0.85;

  // Pastikan teks tidak kepotong kiri kanan dengan memberi padding 10% lebar layar
  // Jadi kita cek panjang teks dan scale font jika terlalu lebar

  const text = "I love you sayang";
  const maxTextWidth = width * 0.8; // 80% lebar canvas
  let measuredWidth = ctx.measureText(text).width;

  if (measuredWidth > maxTextWidth) {
    // Skala font supaya pas maksimal 80% canvas width
  ...
  for (var i = 0; i < Math.PI * 2; i += dr)
    pointsOrigin.push(scaleAndTranslate(heartPosition(i), scaleX * 0.8, scaleY * 0.8, 0, 0));
  for (var i = 0; i < Math.PI * 2; i += dr)
  ...
  function pulse(kx, ky) {
    for (var i = 0; i < pointsOrigin.length; i++) {
      targetPoints[i] = [
        ...
        ky * pointsOrigin[i][1] + (mobile ? height / 2.5 : height / 2.5)
      ];
    }
  }
